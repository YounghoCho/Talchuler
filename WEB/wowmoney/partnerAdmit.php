...
//사진경로를 저장한다.
	...
	if($real['pi_idx']!=''){
		//업데이트
		$sql="update partnerImage set filename='".$path['filename']."', time='".$path['time']."' where p_id='".$_GET['p_id']."'";
		mysql_query($sql);
	}else{
		$sql="insert into partnerImage (pi_idx, p_id, filename, time) 
		values('', '".$_GET['p_id']."', '".$path['filename']."', '".$path['time']."')";
		mysql_query($sql);
	}
//이미지요청을 지운다
