<?
	...
	//메일보내기 S
	$sql="select nickname from user where email='".$_GET['email']."';";
	$q=mysql_query($sql);
	...
	<tr><td colspan="3" style="height:190px;vertical-align:middle;"><br><b>'.$data['nickname'].'님('.$_GET['email'].')</b>의 비밀번호가 <b>'.$password.'</b>로 수정되었습니다.<br>로그인 후 <font color="red">반드시</font> 비밀번호를 수정해주세요</td></tr>
	...
	//실제비밀번호 수정
	$sql="update user set password='".$hash."' where email='".$_GET['email']."'";
	mysql_query($sql);
?>
<script>
